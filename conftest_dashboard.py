    ...
    def send_to_dashboard(self):
        ...
        try:
            response = requests.post(
                f"{DASHBOARD_API}/test-runs", json=self.session_data, timeout=10
            )
            ...
            print(
                "⚠️  Dashboard inte tillgänglig (kör python app.py i dashboard/backend)"
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("suggestions_created", 0) > 0:
                    print(
                        f"🤖 AI skapade {result['suggestions_created']} test-förbättringsförslag!"
                    )
