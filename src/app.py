...
def _get_jwt_user_identifier():
    ...
    if auth_header.startswith("Bearer "):
        ...
        raw = auth_header
    payload = jwt.decode(raw, app.config["SECRET_KEY"], algorithms=["HS256"])
    return payload.get("user") or payload.get("sub") or ""


@app.route("/api/entries/<int:entry_id>/reactions", methods=["GET"])
def get_entry_reactions(entry_id):
    ...
    # Include the current user's own reactions when a valid token is provided
    user_reactions = []
    auth_header = request.headers.get("Authorization", "")
    if auth_header.startswith("Bearer "):
        try:
...
@app.route("/api/entries/<int:entry_id>/reactions", methods=["POST"])
@token_required
