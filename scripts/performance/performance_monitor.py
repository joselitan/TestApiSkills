...
class PerformanceMonitor:
    ...
    def run_performance_test(
        self, duration=60, concurrent_users=5, requests_per_user=10
    ):
        """Run comprehensive performance test"""
        print(
            f"Starting performance test: {duration}s duration, {concurrent_users} users, {requests_per_user} requests/user"
        )
    ...
    def _analyze_results(self, results, system_metrics, duration):
        ...
        if system_metrics:
            cpu_values = [
                m["cpu_percent"] for m in system_metrics if "cpu_percent" in m
            ]
            memory_values = [
    ...
    def _generate_analysis(self):
        ...
        if "response_times" in self.metrics and self.metrics["response_times"]:
            ...
            analysis["response_time"] = {
                "status": (
                    "GOOD"
                    if rt["avg"] < 1.0
                    else "WARNING" if rt["avg"] < 2.0 else "CRITICAL"
                ),
        ...
        if "throughput" in self.metrics:
            ...
            analysis["throughput"] = {
                "status": (
                    "GOOD"
                    if throughput > 10
                    else "WARNING" if throughput > 5 else "CRITICAL"
                ),
        ...
        if "success_rates" in self.metrics:
            ...
            analysis["reliability"] = {
                "status": (
                    ...
                    else "WARNING" if success_rate > 0.90 else "CRITICAL"
                ),
        ...
        if "system_metrics" in self.metrics:
            ...
            analysis["system_resources"] = {
                "cpu_status": (
                    "GOOD"
                    if sys_metrics["cpu_avg"] < 70
                    else "WARNING" if sys_metrics["cpu_avg"] < 85 else "CRITICAL"
                ),
                "memory_status": (
                    "GOOD"
                    if sys_metrics["memory_avg"] < 80
                    else "WARNING" if sys_metrics["memory_avg"] < 90 else "CRITICAL"
                ),
    ...
    def _generate_recommendations(self):
        ...
        if "response_times" in self.metrics and self.metrics["response_times"]:
            ...
            if avg_time > 2.0:
                ...
                recommendations.append(
                    "Monitor response times and consider performance tuning"
                )
        ...
        if "throughput" in self.metrics:
            ...
            if throughput < 10:
                recommendations.append(
                    "Consider scaling application instances or optimizing bottlenecks"
                )
        ...
        if "system_metrics" in self.metrics:
            ...
            if sys_metrics["memory_avg"] > 85:
                recommendations.append(
                    "High memory usage detected, check for memory leaks or increase memory"
                )

        # Error pattern recommendations
        if "error_counts" in self.metrics and self.metrics["error_counts"]:
            recommendations.append(
                "Review error logs and implement appropriate error handling"
            )
    ...
    def print_summary(self):
        ...
        if "system_metrics" in self.metrics:
            ...
            print(f"\nSystem Resources:")
            print(
                f"  CPU Usage: {sys_metrics['cpu_avg']:.1f}% (max: {sys_metrics['cpu_max']:.1f}%)"
            )
            print(
                f"  Memory Usage: {sys_metrics['memory_avg']:.1f}% (max: {sys_metrics['memory_max']:.1f}%)"
            )
...
def main():
    ...
    parser.add_argument(
        "--url", default="http://localhost:5000", help="Base URL to test"
    )
