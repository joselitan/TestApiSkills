...
class DeploymentManager:
    ...
    def deploy_staging(self):
        ...
        result = self.run_command("docker-compose run --rm test-runner pytest test_guestbook_api.py::TestGuestbookAPI::test_login -v", check=False)
    ...
    def deploy_production(self):
        ...
        result = self.run_command("docker-compose run --rm test-runner pytest test_guestbook_api.py -v", check=False)
    ...
    def rollback(self):
        ...
        # Find latest backup
        backups = [f for f in os.listdir("backups/") if f.startswith("guestbook_backup_")]
        if backups:
