...
class GuestbookUser(HttpUser):
    ...
    def read_all_entries(self):
        ...
        if self.token:
            self.client.get("/api/guestbook", headers={
    ...
    def read_with_pagination(self):
        ...
        if self.token:
            self.client.get("/api/guestbook?page=1&limit=10", headers={
                "Authorization": f"Bearer {self.token}"
            })
    ...
    def search_entries(self):
        """Search for entries"""
        if self.token:
            self.client.get("/api/guestbook?search=test", headers={
                "Authorization": f"Bearer {self.token}"
            })
    ...
    def create_entry(self):
        ...
        if self.token:
            self.client.post("/api/guestbook", json={
                "name": "Load Test User",
    ...
    def read_single_entry(self):
        ...
        if self.token:
            ...
            self.client.get("/api/guestbook/1", headers={
                "Authorization": f"Bearer {self.token}"
            })
