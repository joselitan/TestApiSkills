...
def test_read_all_with_search(auth_token):
    ...
    create_resp1 = requests.post(
        f"{BASE_URL}/api/v1/guestbook", json=payload1, headers=headers
    )
    ...
    create_resp2 = requests.post(
        f"{BASE_URL}/api/v1/guestbook", json=payload2, headers=headers
    )
    ...
    print(f"GET /api/v1/guestbook?search={search_term}")
    response = requests.get(
        f"{BASE_URL}/api/v1/guestbook?search={search_term}", headers=headers
    )
...
def test_update_entry(auth_token):
    ...
    response = requests.put(
        f"{BASE_URL}/api/v1/guestbook/{entry_id}", json=update_payload, headers=headers
    )
    assert response.status_code == 200
    data = response.json()
...
def test_delete_entry(auth_token):
    ...
    response = requests.delete(f"{BASE_URL}/api/v1/guestbook/{entry_id}", headers=headers)
    assert response.status_code == 200
    assert response.json()["message"] == "Entry deleted successfully"

    get_response = requests.get(f"{BASE_URL}/api/v1/guestbook/{entry_id}", headers=headers)
    assert get_response.status_code == 404


def test_delete_nonexistent(auth_token):
    headers = {"Authorization": f"Bearer {auth_token}"}
    response = requests.delete(f"{BASE_URL}/api/v1/guestbook/99999", headers=headers)
...
def test_zzz_cleanup_all_test_data(auth_token):
    ...
    # Get all entries
    # simple comment
    response = requests.get(f"{BASE_URL}/api/v1/guestbook?limit=1000", headers=headers)
        ...
        if entries:
            ...
            for entry in entries:
                ...
                delete_resp = requests.delete(
                    f"{BASE_URL}/api/v1/guestbook/{entry_id}", headers=headers
                )
