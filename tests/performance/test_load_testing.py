...
class TestLoadTesting:
    ...
    def test_concurrent_read_load(self):
        ...
        assert statistics.mean(response_times) < 2.0, f"Average response time {statistics.mean(response_times):.2f}s exceeds 2s"
        assert max(response_times) < 5.0, f"Max response time {max(response_times):.2f}s exceeds 5s"
    ...
    def test_mixed_operations_load(self):
        ...
        all_results = []
        with ThreadPoolExecutor(max_workers=concurrent_users) as executor:
            futures = [executor.submit(mixed_user_session) for _ in range(concurrent_users)]
            for future in as_completed(futures):
                all_results.extend(future.result())
        ...
        # Assertions
        assert success_rate >= 0.90, f"Mixed load success rate {success_rate:.2%} below 90%"
        assert avg_response_time < 3.0, f"Mixed load avg response time {avg_response_time:.2f}s exceeds 3s"
    ...
    def test_pagination_load(self):
        ...
        all_results = []
        with ThreadPoolExecutor(max_workers=concurrent_users) as executor:
            futures = [executor.submit(pagination_session) for _ in range(concurrent_users)]
            for future in as_completed(futures):
                all_results.extend(future.result())
        ...
        # Assertions
        assert success_rate >= 0.95, f"Pagination load success rate {success_rate:.2%} below 95%"
        assert statistics.mean(response_times) < 1.5, f"Pagination avg response time {statistics.mean(response_times):.2f}s exceeds 1.5s"
