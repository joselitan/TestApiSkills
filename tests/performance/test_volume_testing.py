...
class TestVolumeTesting:
    ...
    def test_large_dataset_retrieval(self):
        ...
        for _ in range(5):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"Retrieved {entries_count} entries in {end_time - start_time:.3f}s"
                    )
        ...
        if retrieval_times:
            ...
            ), f"Average retrieval time {avg_retrieval_time:.2f}s exceeds 5s"
            assert (
                max_retrieval_time < 10.0
            ), f"Max retrieval time {max_retrieval_time:.2f}s exceeds 10s"
        else:
    ...
    def test_pagination_with_large_dataset(self):
        ...
        for page_size, results in pagination_results.items():
            print(
                f"- Page size {page_size}: avg {results['avg_time']:.3f}s, max {results['max_time']:.3f}s"
            )

            # Assertions
            assert (
                results["avg_time"] < 3.0
            ), f"Page size {page_size} avg time {results['avg_time']:.2f}s exceeds 3s"
            assert (
                results["max_time"] < 5.0
            ), f"Page size {page_size} max time {results['max_time']:.2f}s exceeds 5s"
    ...
    def test_search_performance_large_dataset(self):
        ...
        for term, results in search_results.items():
            print(
                f"- Search '{term}': avg {results['avg_time']:.3f}s, max {results['max_time']:.3f}s"
            )

            # Assertions
            assert (
                results["avg_time"] < 2.0
            ), f"Search '{term}' avg time {results['avg_time']:.2f}s exceeds 2s"
            assert (
                results["max_time"] < 4.0
            ), f"Search '{term}' max time {results['max_time']:.2f}s exceeds 4s"
    ...
    def test_bulk_operations_performance(self):
        ...
        for size, results in bulk_results.items():
            print(
                f"- Bulk size {size}: {results['created_count']}/{size} created in {results['total_time']:.3f}s ({results['rate']:.2f} entries/sec)"
            )

            # Assertions
            assert (
                results["rate"] > 2.0
            ), f"Bulk creation rate {results['rate']:.2f} entries/sec too slow"
