...
class TestSpikeTesting:
        ...
        try:
            if method == "GET":
                ...
                response = requests.post(f"{self.base_url}{endpoint}", 
                                       json=data, headers=self.headers, timeout=15)
    ...
    def test_sudden_user_spike(self):
        ...
        spike_success_rate = sum(1 for r in spike_results if r['success']) / len(spike_results)
        recovery_success_rate = sum(1 for r in recovery_results if r['success']) / len(recovery_results)
        
        normal_avg_time = statistics.mean([r['response_time'] for r in normal_results if r['success']])
        spike_successful = [r for r in spike_results if r['success']]
        spike_avg_time = statistics.mean([r['response_time'] for r in spike_successful]) if spike_successful else 0
        recovery_avg_time = statistics.mean([r['response_time'] for r in recovery_results if r['success']])
        ...
        # Assertions
        assert spike_success_rate >= 0.70, f"Spike success rate {spike_success_rate:.2%} below 70%"
        assert recovery_success_rate >= 0.90, f"Recovery success rate {recovery_success_rate:.2%} below 90%"
        assert recovery_avg_time <= normal_avg_time * 2, "Recovery time too slow compared to normal"
    ...
    def test_traffic_burst_pattern(self):
        ...
        success_rate = sum(1 for r in all_burst_results if r['success']) / len(all_burst_results)
        ...
        # Assertions
        assert success_rate >= 0.75, f"Burst pattern success rate {success_rate:.2%} below 75%"
        assert avg_response_time < 4.0, f"Burst avg response time {avg_response_time:.2f}s exceeds 4s"
    ...
    def test_gradual_spike_buildup(self):
        ...
        for i, stage in enumerate(stages):
            ...
            with ThreadPoolExecutor(max_workers=stage['users']) as executor:
                futures = [executor.submit(stage_session) for _ in range(stage['users'])]
                for future in as_completed(futures):
        ...
        for stage_num, results in stage_results.items():
            print(f"- Stage {stage_num} ({results['users']} users): "
        ...
        for stage_num, results in stage_results.items():
            assert results['success_rate'] >= 0.70, f"Stage {stage_num} success rate {results['success_rate']:.2%} below 70%"
            
        # Peak stage (stage 4) should handle the spike
        peak_results = stage_results[4]
        assert peak_results['success_rate'] >= 0.60, f"Peak spike success rate {peak_results['success_rate']:.2%} below 60%"
    ...
    def test_mixed_operation_spike(self):
        ...
        # Assertions
        assert success_rate >= 0.65, f"Mixed spike success rate {success_rate:.2%} below 65%"
        assert avg_response_time < 5.0, f"Mixed spike avg response time {avg_response_time:.2f}s exceeds 5s"