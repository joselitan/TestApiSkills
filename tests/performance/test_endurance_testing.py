...
class TestEnduranceTesting:
    ...
    def test_sustained_load_endurance(self):
        ...
        with ThreadPoolExecutor(max_workers=concurrent_users) as executor:
            futures = [executor.submit(endurance_user_session) for _ in range(concurrent_users)]
        ...
        # Assertions
        assert success_rate >= 0.95, f"Endurance success rate {success_rate:.2%} below 95%"
        assert avg_response_time < 3.0, f"Endurance avg response time {avg_response_time:.2f}s exceeds 3s"
    ...
    def test_memory_leak_detection(self):
            ...
            while time.time() < end_time:
                ...
                if request_count % 4 == 0:
                    ...
                    data = {
                        ...
                        "comment": f"Memory leak test entry {request_count} " + "X" * 100  # Larger payload
                    }
        ...
        all_results = []
        with ThreadPoolExecutor(max_workers=concurrent_users) as executor:
            futures = [executor.submit(memory_test_session) for _ in range(concurrent_users)]
            for future in futures:
                all_results.extend(future.result())
        ...
        if len(successful_requests) >= 10:
            ...
            for i in range(5):
                ...
                end_idx = start_idx + segment_size if i < 4 else len(successful_requests)
            ...
            increasing_trend = all(segments[i] <= segments[i+1] * 1.2 for i in range(len(segments)-1))
            max_increase = (max(segments) - min(segments)) / min(segments) * 100 if segments else 0
            ...
            # Assertions
            assert max_increase < 100, f"Response time increase {max_increase:.1f}% suggests memory leak"
            assert not (increasing_trend and max_increase > 50), "Consistent response time increase suggests memory leak"
    ...
    def test_connection_pool_endurance(self):
        ...
        success_rate = sum(1 for r in all_results if r['success']) / len(all_results)
        connection_errors = sum(1 for r in all_results if not r['success'] and 'connection' in str(r.get('error', '')).lower())
        timeout_errors = sum(1 for r in all_results if not r['success'] and 'timeout' in str(r.get('error', '')).lower())
        ...
        # Assertions
        assert success_rate >= 0.90, f"Connection pool success rate {success_rate:.2%} below 90%"
        assert connection_errors < len(all_results) * 0.05, f"Too many connection errors: {connection_errors}"
    ...
    def test_gradual_load_increase_endurance(self):
        ...
        for i, phase in enumerate(phases):
            ...
            with ThreadPoolExecutor(max_workers=phase['users']) as executor:
                futures = [executor.submit(phase_session) for _ in range(phase['users'])]
                for future in futures:
        ...
        for phase_num, results in phase_results.items():
            print(f"- Phase {phase_num} ({results['users']} users): "
        ...
        response_times = [results['avg_response_time'] for results in phase_results.values()]
        success_rates = [results['success_rate'] for results in phase_results.values()]
        
        # Assertions
        for phase_num, results in phase_results.items():
            assert results['success_rate'] >= 0.85, f"Phase {phase_num} success rate {results['success_rate']:.2%} below 85%"
        
        # Final phase should still maintain reasonable performance
        final_phase = phase_results[len(phases)]
        assert final_phase['success_rate'] >= 0.80, f"Final phase success rate {final_phase['success_rate']:.2%} below 80%"
        assert final_phase['avg_response_time'] < 4.0, f"Final phase response time {final_phase['avg_response_time']:.2f}s too slow"