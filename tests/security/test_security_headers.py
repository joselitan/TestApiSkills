...
class TestSecurityHeaders:
    ...
    def test_security_headers_presence(self):
        ...
        expected_headers = {
            ...
            "X-XSS-Protection": [
                "1; mode=block",
                "0",
            ],  # 0 is also acceptable (modern approach)
                ...
                if header_value is None:
                    ...
                    if expected_values and header_value not in expected_values:
                        if (
                            ...
                            and "max-age=" not in header_value
                        ):
    ...
    def test_content_security_policy(self):
        ...
        if csp_header:
            with allure.step("Analyzing CSP directives"):
                ...
                if csp_issues:
                    allure.attach(
                        f"CSP issues: {csp_issues}",
                        name="CSP Security Issues",
                        attachment_type=allure.attachment_type.TEXT,
                    )
    ...
    def test_https_enforcement(self):
        ...
        try:
            http_response = requests.get(
                "http://localhost:8080/", allow_redirects=False
            )
            ...
            if http_response.status_code in [301, 302, 307, 308]:
                ...
                if location.startswith("https://"):
                    ...
                    allure.attach(
                        "HTTP redirect but not to HTTPS",
                        name="Weak HTTPS Redirect",
                        attachment_type=allure.attachment_type.TEXT,
                    )
            else:
                allure.attach(
            ...
            allure.attach(
                "Could not test HTTP endpoint",
                name="HTTPS Test Skipped",
                attachment_type=allure.attachment_type.TEXT,
            )
    ...
    def test_clickjacking_protection(self):
        ...
        if x_frame_options:
            if x_frame_options.upper() in ["DENY", "SAMEORIGIN"]:
                allure.attach(
                    f"Good X-Frame-Options: {x_frame_options}",
                    name="Clickjacking Protection",
                    attachment_type=allure.attachment_type.TEXT,
                )
            else:
                allure.attach(
                    f"Weak X-Frame-Options: {x_frame_options}",
            ...
            if "frame-ancestors" in csp_header:
                allure.attach(
                    "CSP frame-ancestors found (alternative to X-Frame-Options)",
                    name="CSP Clickjacking Protection",
                    attachment_type=allure.attachment_type.TEXT,
                )
            else:
                allure.attach(
    ...
    def test_information_disclosure(self):
        ...
        if disclosed_info:
            allure.attach(
                f"Information disclosure: {disclosed_info}",
                name="Information Disclosure",
                attachment_type=allure.attachment_type.TEXT,
            )
        else:
            allure.attach(
                "No sensitive information disclosed in headers",
                name="Good Information Hiding",
                attachment_type=allure.attachment_type.TEXT,
            )
    ...
    def test_mime_sniffing_protection(self):
        ...
        if x_content_type_options == "nosniff":
            allure.attach(
                ...
                name="MIME Protection",
                attachment_type=allure.attachment_type.TEXT,
            )
        else:
            allure.attach(
                "No MIME sniffing protection",
                name="Missing MIME Protection",
                attachment_type=allure.attachment_type.TEXT,
            )
    ...
    def test_referrer_policy(self):
        ...
        if referrer_policy:
            ...
            if referrer_policy in secure_policies:
                allure.attach(
                    f"Secure referrer policy: {referrer_policy}",
                    name="Good Referrer Policy",
                    attachment_type=allure.attachment_type.TEXT,
                )
            else:
                allure.attach(
                    f"Potentially weak referrer policy: {referrer_policy}",
    ...
    def test_api_security_headers(self, auth_token):
        ...
        if api_security_issues:
            allure.attach(
                f"API security issues: {api_security_issues}",
                name="API Security Headers",
                attachment_type=allure.attachment_type.TEXT,
            )
        else:
            allure.attach(
                "API security headers look good",
                name="Good API Security",
                attachment_type=allure.attachment_type.TEXT,
            )
    ...
    def test_error_page_disclosure(self):
        ...
        for endpoint in error_endpoints:
            with allure.step(f"Testing error page: {endpoint}"):
                ...
                if disclosed_info:
                    allure.attach(
                        f"Error page disclosure in {endpoint}: {disclosed_info}",
                        name="Error Page Disclosure",
                        attachment_type=allure.attachment_type.TEXT,
                    )
