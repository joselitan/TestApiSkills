...
class TestAuthenticationFailures:
    ...
    def test_brute_force_protection(self):
        ...
        with allure.step("Attempting multiple failed logins"):
            for i in range(max_attempts):
                ...
                if response.status_code == 401:
                    ...
                    allure.attach(f"Rate limiting triggered after {failed_attempts} attempts", 
        ...
        allure.attach(f"Total failed attempts before rate limiting: {failed_attempts}", 
                     name="Brute Force Results", attachment_type=allure.attachment_type.TEXT)
        ...
        for weak_password in weak_passwords:
            with allure.step(f"Testing weak password: {weak_password}"):
                ...
                if response.status_code == 200 and weak_password == "password123":
                    allure.attach("System uses weak default password", 
                                name="Weak Password Found", attachment_type=allure.attachment_type.TEXT)
    ...
    def test_jwt_token_security(self, auth_token):
        ...
        for token in malformed_tokens:
            with allure.step(f"Testing malformed token: {token[:20]}..."):
                test_headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
                response = requests.get(f"{BASE_URL}/api/guestbook", headers=test_headers)
                assert response.status_code == 401, f"Malformed token should be rejected: {token[:20]}..."
    ...
    def test_token_expiration(self):
        ...
        allure.attach("Token lifetime: 24 hours", 
                     name="Token Configuration", attachment_type=allure.attachment_type.TEXT)
    ...
    def test_session_management(self, auth_token):
        ...
        with allure.step("Testing concurrent sessions"):
            ...
            allure.attach("Multiple concurrent sessions allowed (JWT stateless)", 
                         name="Session Behavior", attachment_type=allure.attachment_type.TEXT)
        ...
        for endpoint in reset_endpoints:
            with allure.step(f"Testing password reset endpoint: {endpoint}"):
                ...
                if response.status_code != 404:
                    allure.attach(f"Password reset endpoint found: {endpoint}", 
                                name="Password Reset Discovery", attachment_type=allure.attachment_type.TEXT)
    ...
    def test_account_enumeration(self):
        ...
        allure.attach(f"Response times: {response_times}", 
                     name="Timing Analysis", attachment_type=allure.attachment_type.TEXT)
        allure.attach(f"Response messages: {response_messages}", 
                     name="Message Analysis", attachment_type=allure.attachment_type.TEXT)
        
        # Significant timing difference could indicate enumeration vulnerability
        if time_difference > 0.1:  # 100ms difference
            allure.attach(f"Potential timing-based enumeration: {time_difference:.3f}s difference", 
                         name="Enumeration Risk", attachment_type=allure.attachment_type.TEXT)
    ...
    def test_mfa_implementation(self):
        ...
        with allure.step("Checking for MFA implementation"):
            ...
            if response.status_code == 200:
                ...
                allure.attach("No MFA implementation detected", 
                             name="MFA Status", attachment_type=allure.attachment_type.TEXT)
            elif "mfa" in response.json().get("message", "").lower():
                allure.attach("MFA implementation detected", 
                             name="MFA Status", attachment_type=allure.attachment_type.TEXT)