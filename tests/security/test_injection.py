        ...
        for payload in sql_payloads:
            with allure.step(f"Testing SQL injection in search: {payload}"):
                ...
                if response.status_code == 400:
                    ...
                    assert "Invalid search query" in response.json().get(
                        "message", ""
                    ), f"Unexpected error message for payload: {payload}"
            ...
            with allure.step(f"Testing XSS payload: {payload}"):
                ...
                if response.status_code == 201:
                    ...
                    if get_response.status_code == 200:
                        ...
                        assert "<script>" not in entry_data.get(
                            "name", ""
                        ), "XSS in name field"
                        assert "<script>" not in entry_data.get(
                            "email", ""
                        ...
                        assert "javascript:" not in entry_data.get(
                            "name", ""
                        ), "JavaScript protocol in name"
                        assert "javascript:" not in entry_data.get(
                            "email", ""
                        ), "JavaScript protocol in email"
                        assert "javascript:" not in entry_data.get(
                            "comment", ""
                        ), "JavaScript protocol in comment"
        ...
        for filename in malicious_filenames:
            with allure.step(f"Testing command injection with filename: {filename}"):
                ...
                assert response.status_code in [
                    ...
                    404,
                    422,
                ], f"Command injection possible with filename: {filename}"
        ...
        for payload in nosql_payloads:
            with allure.step(f"Testing NoSQL injection: {payload}"):
                ...
                assert response.status_code in [
                    400,
                    401,
                ], f"NoSQL injection possible: {payload}"
                assert (
                    "token" not in response.json()
                ), f"Authentication bypassed: {payload}"
        ...
        for payload in ldap_payloads:
            with allure.step(f"Testing LDAP injection: {payload}"):
                ...
                assert response.status_code in [
                    400,
                    401,
                ], f"LDAP injection possible: {payload}"
