...
DASHBOARD_API = "http://127.0.0.1:6001/api"
...
class DashboardReporter:
    ...
    def pytest_collection_finish(self, session):
        ...
        print(
            f"Detekterad test-typ: {test_type} (sanity check: found {sorted(detected_types)})"
        )
        ...
        if report.when == "call":
            test_case = {
                ...
                "status": (
                    "passed"
                    if report.passed
                    else "failed" if report.failed else "skipped"
                ),
    ...
    def get_coverage_percentage(self):
        if self.session_data["total_tests"] > 0:
            success_rate = (
                self.session_data["passed_tests"] / self.session_data["total_tests"]
            ) * 100
    ...
    def send_individual_result(self, test_case):
        ...
        try:
            ...
            status_emoji = (
                ...
                else "FAIL" if test_case["status"] == "failed" else "SKIP"
            )
    ...
    def send_to_dashboard(self):
        try:
            response = requests.post(
                f"{DASHBOARD_API}/test-runs", json=self.session_data, timeout=5
            )
            ...
            print(
                "⚠️  Dashboard inte tillgänglig (starta: cd dashboard/backend && python app.py)"
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("suggestions_created", 0) > 0:
                    print(
                        f"🤖 AI skapade {result['suggestions_created']} förbättringsförslag!"
                    )
        except:
