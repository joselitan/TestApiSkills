...
class QAPlatformTester:
    ...
    def test_api_crud(self, token):
        ...
        try:
            ...
            response = requests.post(f"{self.base_url}/api/guestbook", json=entry_data, headers=headers, timeout=5)
            ...
            if response.status_code == 201:
                ...
                # Test READ
                response = requests.get(f"{self.base_url}/api/guestbook/{entry_id}", headers=headers, timeout=5)
                if response.status_code == 200:
                ...
                # Test LIST
                response = requests.get(f"{self.base_url}/api/guestbook", headers=headers, timeout=5)
                if response.status_code == 200:
                ...
                # Test DELETE (cleanup)
                response = requests.delete(f"{self.base_url}/api/guestbook/{entry_id}", headers=headers, timeout=5)
                if response.status_code == 200:
    ...
    def test_environment_config(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.test_results.append(f"❌ Environment check failed: {response.status_code}")
    ...
    def generate_report(self, environment):
        ...
        with open(f"reports/test_run_{environment}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json", "w") as f:
