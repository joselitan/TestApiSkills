...
from models import AITestSuggestion, DatabaseManager
...
def analyze_coverage_gaps(coverage_data: Dict) -> List[Dict]:
    ...
    for file_path, file_coverage in coverage_data.items():
        ...
        if uncovered_lines and total_lines > 0:
            coverage_percentage = (
                (total_lines - len(uncovered_lines)) / total_lines
            ) * 100

            priority = (
                "high"
                if coverage_percentage < 70
                else "medium" if coverage_percentage < 85 else "low"
            )
...
def detect_flaky_tests(test_runs: List[Dict]) -> List[str]:
    ...
    for test_name, results in test_results.items():
        if (
            len(set(results)) > 1 and len(results) >= 3
        ):  # Mixed results in multiple runs
